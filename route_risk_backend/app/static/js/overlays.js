...
function buildSegPopup(seg, pt) {
  ...
  var stopD = hgvStopDist(seg.speed_limit_kmh, seg.surface_condition, seg.friction_condition);
  if (stopD !== null) h += '<tr><td>&#x1F6D1; HGV stop dist.</td><td>' + stopD + ' m</td></tr>';
  ...
  if (seg.recent_maintenance_task)
    h += '<tr><td>&#x1F69C; Winter treatment</td><td>' + esc(seg.recent_maintenance_task.replace(/_/g, ' ').toLowerCase()) +
         ' &middot; ' + seg.recent_maintenance_age_minutes + ' min ago &middot; ' + seg.recent_maintenance_distance_m + ' m</td></tr>';
